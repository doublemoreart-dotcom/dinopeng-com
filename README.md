...
本 repo 保留 `dinopeng.com` 的唯一 `CNAME` 與 GitHub Pages 發布責任。`.github/workflows/sync-projects.yml` 每小時及手動執行時，會先依 `config/project-sources.json` 比對各來源分支與 `.project-sync-state.json` 的發布版本。來源沒有變更時會直接結束；有變更時只下載、檢查、同步及提交變更的專案。只有 `taiwan-food-safety` 自己更新時才安裝 Node.js 並重建 Next.js 靜態輸出。Actions 執行摘要會列出每個來源的 ref、SHA 與更新狀態。

若需要排除來源版本判斷、完整重建並驗證所有專案，可在 Actions → **Sync project sites** → **Run workflow** 勾選 `force_sync`。新增或調整來源 repo 時，先修改 `config/project-sources.json`；同步腳本仍負責各專案的發布檔案允許清單。同步腳本接受包含各來源 checkout 的共同目錄、portal 目錄，以及逗號分隔的專案 ID，例如只同步 CCP Stability Spending：
